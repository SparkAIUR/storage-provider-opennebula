...
func TestNodeRecordsAndClearsLocalDeviceMissingReport(t *testing.T) {
	...
	require.Error(t, ns.confirmLocalDeviceRecovery(context.Background(), &report, "vol-1", "/dev/sdd", nil, publishContext))
	retained, exists := ns.currentLocalDeviceReport(context.Background(), "vol-1")
	require.True(t, exists)
	require.Equal(t, report, retained)
	observed := &LocalDiskIdentity{ObservedFromDevice: &LocalDiskObservedIdentity{Block: &LocalDiskObservedBlockIdentity{DeviceSerial: "onecsi-439"}}}
	require.NoError(t, ns.confirmLocalDeviceRecovery(context.Background(), &report, "vol-1", "/dev/sdd", observed, publishContext))
	cm, err = driver.kubeRuntime.client.CoreV1().ConfigMaps(namespaceFromServiceAccount()).Get(context.Background(), localDeviceStateConfigMapName, metav1.GetOptions{})
	require.NoError(t, err)
